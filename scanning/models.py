...
class ScanJob(models.Model):
    ...
    job_type = models.CharField(
        max_length=20,
        choices=[('manual', 'Manual'), ('scheduled', 'Scheduled'), ('triggered', 'Triggered')],
        default='manual',
    )
    ...
    def __str__(self):
        return f"Scan Job {self.id} - {self.platform.display_name} ({self.status})"
...
class ScannedContent(models.Model):
    ...
    # ✅ make scan_job optional so manual/API inserts without a job don't fail
    scan_job = models.ForeignKey(
        ScanJob, on_delete=models.SET_NULL, null=True, blank=True, related_name='scanned_content'
    )
    ...
    # keep a fast lookup; we’ll compute automatically if missing
    ...
    def __str__(self):
        return f"{self.platform.display_name} - {self.title or self.platform_content_id}"
...
class ScanSchedule(models.Model):
    ...
    frequency = models.CharField(
        max_length=20, choices=[('daily', 'Daily'), ('weekly', 'Weekly'), ('monthly', 'Monthly')]
    )
    is_active = models.BooleanField(default=True)
